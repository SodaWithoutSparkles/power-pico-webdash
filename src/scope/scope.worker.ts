...
onmessage = async (e: MessageEvent<WorkerRequest>) => {
    ...
    switch (msg.type) {
        ...
        case "get-data-window": {
            ...
            const resp = {
                type: "window-data" as const,
                payload: data,
                _req: { _t0: (msg as any)._t0 },
            };
            self.postMessage(resp, bucketedTransferables(data));
            const t4 = performance.now();
            console.log(
                "[perf] worker bucket=" + msg.bucketCount +
                " ringSamples=" + ringLen +
                " bucketTime=" + (t3 - t1).toFixed(1) + "ms" +
                " postMsgTime=" + (t4 - t3).toFixed(1) + "ms"
            );
            break;
        }
